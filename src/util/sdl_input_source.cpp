...
std::unique_ptr<ForceFeedbackDevice> SDLInputSource::CreateForceFeedbackDevice(const std::string_view& device)
{
  SDL_Joystick* joystick = GetJoystickForDevice(device);
  if (!joystick)
  {
    Log_ErrorFmt("No SDL_Joystick for {}. Cannot use FF.", device);
    return nullptr;
  }
  ...
  if (!haptic)
  {
    Log_ErrorFmt("Haptic is not supported on {} ({}).", device, SDL_JoystickName(joystick));
    return nullptr;
  }

  return std::unique_ptr<SDLFFDevice>(new SDLFFDevice(joystick, haptic));
}

SDLFFDevice::SDLFFDevice(SDL_Joystick* joystick, SDL_Haptic* haptic) : m_haptic(haptic)
{
...
void SDLFFDevice::CreateEffects(SDL_Joystick* joystick)
{
  ...
  if (supported & SDL_HAPTIC_CONSTANT)
  {
    ...
    if (m_constant_effect_id < 0)
      Log_ErrorFmt("SDL_HapticNewEffect() for constant failed: {}", SDL_GetError());
  }
  else
  {
    Log_WarningFmt("Constant effect is not supported on '{}'", SDL_JoystickName(joystick));
  }
  ...
  if (supported & SDL_HAPTIC_SPRING)
  {
    ...
    if (m_spring_effect_id < 0)
      Log_ErrorFmt("SDL_HapticNewEffect() for spring failed: {}", SDL_GetError());
  }
  else
  {
    Log_WarningFmt("Spring effect is not supported on '{}'", SDL_JoystickName(joystick));
  }
  ...
  if (supported & SDL_HAPTIC_DAMPER)
  {
    ...
    if (m_damper_effect_id < 0)
      Log_ErrorFmt("SDL_HapticNewEffect() for damper failed: {}", SDL_GetError());
  }
  else
  {
    Log_WarningFmt("Damper effect is not supported on '{}'", SDL_JoystickName(joystick));
  }
  ...
  if (supported & SDL_HAPTIC_FRICTION)
  {
    ...
    if (m_friction_effect_id < 0)
      Log_ErrorFmt("SDL_HapticNewEffect() for friction failed: {}", SDL_GetError());
  }
  else
  {
    Log_WarningFmt("Friction effect is not supported on '{}'", SDL_JoystickName(joystick));
  }

  m_autocenter_supported = (supported & SDL_HAPTIC_AUTOCENTER) != 0;
  if (!m_autocenter_supported)
    Log_WarningFmt("Autocenter effect is not supported on '{}'", SDL_JoystickName(joystick));
}
...
void SDLFFDevice::SetConstantForce(int level)
{
  ...
  if (m_constant_effect.constant.level != new_level)
  {
    ...
    if (SDL_HapticUpdateEffect(m_haptic, m_constant_effect_id, &m_constant_effect) != 0)
      Log_ErrorFmt("SDL_HapticUpdateEffect() for constant failed: {}", SDL_GetError());
  }
  ...
  if (!m_constant_effect_running)
  {
    if (SDL_HapticRunEffect(m_haptic, m_constant_effect_id, SDL_HAPTIC_INFINITY) == 0)
      ...
      Log_ErrorFmt("SDL_HapticRunEffect() for constant failed: {}", SDL_GetError());
  }
}
...
void SDLFFDevice::SetSpringForce(const EffectData& ff)
{
  if (m_spring_effect_id < 0)
    return;

  m_spring_effect.condition.left_sat[0] = ClampU16(ff.left_saturation);
  m_spring_effect.condition.left_coeff[0] = ClampS16(ff.left_coeff);
  m_spring_effect.condition.right_sat[0] = ClampU16(ff.right_saturation);
  m_spring_effect.condition.right_coeff[0] = ClampS16(ff.right_coeff);
  m_spring_effect.condition.deadband[0] = ClampU16(ff.deadband);
  m_spring_effect.condition.center[0] = ClampS16(ff.center);

  if (SDL_HapticUpdateEffect(m_haptic, m_spring_effect_id, &m_spring_effect) != 0)
    Log_ErrorFmt("SDL_HapticUpdateEffect() for spring failed: {}", SDL_GetError());
  ...
  if (!m_spring_effect_running)
  {
    if (SDL_HapticRunEffect(m_haptic, m_spring_effect_id, SDL_HAPTIC_INFINITY) == 0)
      ...
      Log_ErrorFmt("SDL_HapticRunEffect() for spring failed: {}", SDL_GetError());
  }
}

void SDLFFDevice::SetDamperForce(const EffectData& ff)
{
  if (m_damper_effect_id < 0)
    return;

  m_damper_effect.condition.left_sat[0] = ClampU16(ff.left_saturation);
  m_damper_effect.condition.left_coeff[0] = ClampS16(ff.left_coeff);
  m_damper_effect.condition.right_sat[0] = ClampU16(ff.right_saturation);
  m_damper_effect.condition.right_coeff[0] = ClampS16(ff.right_coeff);
  m_damper_effect.condition.deadband[0] = ClampU16(ff.deadband);
  m_damper_effect.condition.center[0] = ClampS16(ff.center);

  if (SDL_HapticUpdateEffect(m_haptic, m_damper_effect_id, &m_damper_effect) != 0)
    Log_ErrorFmt("SDL_HapticUpdateEffect() for damper failed: {}", SDL_GetError());
  ...
  if (!m_damper_effect_running)
  {
    if (SDL_HapticRunEffect(m_haptic, m_damper_effect_id, SDL_HAPTIC_INFINITY) == 0)
      ...
      Log_ErrorFmt("SDL_HapticRunEffect() for damper failed: {}", SDL_GetError());
  }
}

void SDLFFDevice::SetFrictionForce(const EffectData& ff)
{
  if (m_friction_effect_id < 0)
    return;

  m_friction_effect.condition.left_sat[0] = ClampU16(ff.left_saturation);
  m_friction_effect.condition.left_coeff[0] = ClampS16(ff.left_coeff);
  m_friction_effect.condition.right_sat[0] = ClampU16(ff.right_saturation);
  m_friction_effect.condition.right_coeff[0] = ClampS16(ff.right_coeff);
  m_friction_effect.condition.deadband[0] = ClampU16(ff.deadband);
  m_friction_effect.condition.center[0] = ClampS16(ff.center);
  ...
  if (SDL_HapticUpdateEffect(m_haptic, m_friction_effect_id, &m_friction_effect) != 0)
  {
    if (!m_friction_effect_running && SDL_HapticRunEffect(m_haptic, m_friction_effect_id, SDL_HAPTIC_INFINITY) == 0)
      ...
      Log_ErrorFmt("SDL_HapticUpdateEffect() for friction failed: {}", SDL_GetError());
  }
}
...
void SDLFFDevice::SetAutoCenter(int value)
{
  if (m_autocenter_supported)
  {
    if (SDL_HapticSetAutocenter(m_haptic, value) != 0)
      Log_ErrorFmt("SDL_HapticSetAutocenter() failed: {}", SDL_GetError());
  }
}
