...
static void FormatLogMessageAndPrint(MessageCategory cat, const char* functionName, std::string_view message,
                                     bool timestamp, bool ansi_color_code, bool newline, const T& callback);
#ifdef _WIN32
template<typename T>
...
ALWAYS_INLINE_RELEASE void Log::FormatLogMessageForDisplay(fmt::memory_buffer& buffer, MessageCategory cat,
                                                           ...
                                                           bool timestamp, bool ansi_color_code, bool newline)
{
  ...
  if (timestamp)
  {
    ...
    {
      fmt::format_to(appender, "[{:10.4f}] {}{}/{}: {}{}{}", message_time, color_start,
                     s_log_level_characters[static_cast<size_t>(level)], channel_name, message, color_end, message_end);
    }
  }
  else
  {
    ...
    {
      fmt::format_to(appender, "{}{}/{}: {}{}{}", color_start, s_log_level_characters[static_cast<size_t>(level)],
                     channel_name, message, color_end, message_end);
    }
  }
}
                                                         ...
                                                         bool newline, const T& callback)
{
  ...
  Log::FormatLogMessageForDisplay(buffer, cat, functionName, message, timestamp, ansi_color_code, newline);
                                   ...
                                   std::string_view message)
{
  ...
  FormatLogMessageAndPrintW(
    cat, functionName, message, s_state.console_output_timestamps, true, true, [cat](const std::wstring_view& message) {
  ...
  FormatLogMessageAndPrint(
    cat, functionName, message, s_state.console_output_timestamps, true, true, [cat](std::string_view message) {
...
#if defined(_WIN32)
  FormatLogMessageAndPrintW(cat, functionName, message, false, false, true,
                                ...
                                std::string_view message)
{
  ...
  FormatLogMessageAndPrint(cat, functionName, message, true, false, true, [](std::string_view message) {
